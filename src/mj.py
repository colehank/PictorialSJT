...
class ImageGenerator:
   ...
   def generate_image(
      ...
      prompt_input = f"{prompt} --v {model} --s {stylize} --seed {seed} --chaos {chaos}"
      if c_ref:
   ...
   def action(self, task_id: str, action: str, img_id: int):
      ...
      response = requests.post(MJConfig.img_change_url, headers=self.headers, data=payload)
   ...
   def upload_image(self, image: str | Image.Image) -> str:
      ...
      response = requests.post(MJConfig.img_upload_url, headers=self.headers, data=payload)
